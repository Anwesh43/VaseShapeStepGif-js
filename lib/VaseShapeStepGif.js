...
const drawVSSNode = (context, i, scale) => {
    //console.log(`drawing node ${scale}`)
...
class VSSNode {
    ...
    draw(context) {
        drawVSSNode(context, this.i, this.state.scale)
        if (this.next) {
    ...
    update(cb) {
        this.curr.update(() => {
            ...
            if (this.curr.i == 0 && this.dir == 1) {
                ...
                if (this.dir == 1) {
                    console.log(`completed animation number ${this.curr.i - this.dir}`)
                } else {

                    console.log(`completed reset of animation number ${this.curr.i - this.dir}`)
                }
                this.startUpdating()
            }
        })
...
class VaseShapeStepGif {
    ...
    create(fn) {
        ...
        console.log("started creating gif")
    ...
    static init(fn) {
        ...
        const vss = new VaseShapeStepGif()
        console.log("initialized gifencoder")
        vss.create(fn)
    }
}
